...
def extract_relations(net, params):
    ...
    if params.binary_pos_only:
        ...
        attention_mask_continous = np.reshape((np.tanh(attention_continous)+1)/2, [-1, params.n_rules*params.n_classes])
...
if __name__ == '__main__':
    ...
    # Data Loading (to calculate the variables' mean in the train features)
    #dataset = load_dataset.generate_gaussian_data_complicated_function(n_samples=400, mislabel=0, random_state=random_state)
    
    model_dir = sys.argv[1]
    # model_data = model_dir.split('_GFN')[0] + '.csv'
    # dataset = load_dataset.load_umhs(f'data_files/{model_data}')
    dataset = pickle.load(open(f'cv_results/{model_dir}/dataset.pkl', 'rb'))

    classifier_path = f'cv_results/{model_dir}/saved_test_model_GFN.mdl'
    # Read a list of trained classifiers from k-fold cross-validation.
    classifiers = pickle.load(open(classifier_path, 'rb'))
